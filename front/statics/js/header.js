function renderHeader(){
	if(localStorage.getItem("log")=="true"){
		...
		if(localStorage.getItem("cUserKind")=="1"){
			adminLog();
		}
		else{
			renderLog();
...
function adminLog(){
	...
	console.log('Header establecido como ADMIN.');
}
...
function renderLog(){
	...
	$("#mainHeader").append(
		'<ul class="memenu skyblue text-center">\
			<li class="active">\
			<button class="btn-evento" onclick="userSessions();" data-toggle="modal" data-target="#myModalListSessionsUser">SESIONES</button>\
			<button class="btn-evento" onclick="LogoutUser();" data-toggle="modal" data-target="#"><span class="glyphicon glyphicon-log-out" aria-hidden="true"></span></button>\
			</li>\
		</ul>'
		);
